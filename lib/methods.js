    ...
    if (typeof option === 'object') {
        ...
        for (const key in option) {
            ...
            if (typeof e === 'object') {
                ...
                //@ts-ignore
                f = (0, index_1.field)(key) + ' as ' + (0, index_1.field)(e);
            }
            r.push(f);
        }
        return r.join(', ');
...
function where(option, op = 'AND') {
        ...
        if (option.length === 2) {
            ...
            if (op === 'in' || op === 'not in') {
                if (!Array.isArray(option[2]))
                    throw new index_1.FlqError(`methods.field: "${op}"比较符仅支持数组,不受支持的参数类型:${JSON.stringify(option[2])}`);
                return `${(0, index_1.field)(option[0])} ${op} (${option[2].map((e) => (0, index_1.escape)(e)).join(', ')})`;
            }
            //@ts-ignore
            op = op.toUpperCase();
    ...
    if (typeof option === 'object') {
        ...
        for (const key in option) {
            ...
            let pk = (0, index_1.field)(key);
            if (value instanceof Array) {
                ws.push(`${pk} IN (${value.map((e) => (0, index_1.escape)(e)).join(', ')})`);
                continue;
            }
            ws.push(`${pk} = ${(0, index_1.escape)(value)}`);
        }
        return ws.join(' ' + op + ' ');
    }
